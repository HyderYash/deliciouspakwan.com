...
class Albums{
	...
	public function generateNewAlbumAccessKey() {
			...
			$sql = "SELECT ID  
				...
				if($chkSqlRows == 0){
					//print 'chkSqlRows is zero';
						$qry = "INSERT INTO dp_albums (`ID`, `IMAGE_THUMB_URL`, `ALBUM_FOLDER_TITLE`, `ALBUM_DISPLAY_TITLE`,`ALBUM_DESC`, `UPLOADED_DATE`, `STATUS`, `IS_DP_ALBUMS`, `ALBUM_TYPE`, `ALBUM_ACCESS_KEY`, `EXPIRE_COUNTER`, `VISIT_COUNTER` )  VALUES 
						(NULL, '" . $postData->IMAGE_THUMB_URL . "', '" . trim($postData->ALBUM_FOLDER_TITLE) . "', '" . trim($postData->ALBUM_DISPLAY_TITLE) . "', '" . addslashes($postData->DESC) . "', '" . $postData->UPLOADED_DATE . "','" . $postData->STATUS . "', 'N', '" . $postData->ALBUM_TYPE . "', '" . $this->generateNewAlbumAccessKey() . "', 0, 0)";
						//print($qry);
						$ptr = 	$this->db->get_sql_exec($qry);
				}
				if($chkSqlRows == 1){
					//print 'chkSqlRows is one';
					$updatePtr = $this->db->set_multiple_fields('dp_albums', 'IMAGE_THUMB_URL = "' . $postData->IMAGE_THUMB_URL . '",ALBUM_DISPLAY_TITLE = "' . $postData->ALBUM_DISPLAY_TITLE . '",ALBUM_DESC = "' . addslashes($postData->DESC) . '",STATUS = "' . $postData->STATUS . '",ALBUM_TYPE = "' . $postData->ALBUM_TYPE . '"', 'LOWER(ALBUM_FOLDER_TITLE) = "' . strtolower(trim($postData->ALBUM_FOLDER_TITLE)) . '"');
				}
		}
		return true;
	}
	...
	public function getAlbumsList($postData){
		...
		$sql = "SELECT
				  ...
				  dp_albums.STATUS,
				  dp_albums.ALBUM_TYPE,
				  dp_albums.ALBUM_ACCESS_KEY,
				  dp_albums.VISIT_COUNTER,
				  SUM(CASE WHEN dp_albums_photos.FILE_TYPE = 'photo' THEN 1 ELSE 0 END) AS photoCount,
				  SUM(CASE WHEN dp_albums_photos.FILE_TYPE = 'video' THEN 1 ELSE 0 END ) AS videoCount
				FROM
				  dp_albums
				LEFT JOIN dp_albums_photos ON dp_albums.ID = dp_albums_photos.ALBUM_ID";
		if($postData->DISPLAY == 'FE'){
			$sql .= " AND dp_albums_photos.PHOTO_STATUS = 'Y'";
			$sql .= " WHERE dp_albums.STATUS = 'Y'";
		}
		$sql .= " GROUP BY dp_albums.ID,dp_albums.`ALBUM_FOLDER_TITLE`,dp_albums.`ALBUM_DISPLAY_TITLE` ORDER BY dp_albums.ALBUM_TYPE ASC, dp_albums.ID DESC";
		$chkSqlPtr = $this->db->get_sql_exec($sql);
		$chkSqlRows = $this->db->get_db_num_rows($chkSqlPtr);
		if($chkSqlRows > 0){				
	...
	public function getAlbumAccessDetails() {
		$sql = "SELECT * FROM dp_album_key";
		$chkSqlPtr = $this->db->get_sql_exec($sql);
		$chkSqlRows = $this->db->get_db_num_rows($chkSqlPtr);
		$result = $this->db->get_one_record($sql);
		return $result;
	}
	public function changeAlbumAccessDetails($postData) {
		$updatePtr = $this->db->set_multiple_fields('dp_album_key', 'ADMIN_KEY = "' . $postData->ADMIN_KEY . '", ACCESS_LIMIT = "' . $postData->ACCESS_LIMIT . '"', 'ID > 0');
		if($updatePtr > 0){
			return true;
		}else{
			return false;
	...
	public function checkHomePageAccesskey($postData) {
		$sql = "SELECT * FROM dp_album_key WHERE ADMIN_KEY = " . $postData->ADMIN_KEY;
		$chkSqlPtr = $this->db->get_sql_exec($sql);
		$chkSqlRows = $this->db->get_db_num_rows($chkSqlPtr);
		if($chkSqlRows == 1) {
			return true;
	...
	public function updateAlbumsViewCounter() {
		$qry = "SELECT ID, ALBUMS_VIEW_COUNTER FROM dp_album_key WHERE ID > 0";
		$chkResult = $this->db->get_one_record($qry);
		$updatePtr = $this->db->set_multiple_fields('dp_album_key', 'ALBUMS_VIEW_COUNTER = "' . ($chkResult['ALBUMS_VIEW_COUNTER'] + 1) . '"', 'ID > 0');
		$newchkResult = $this->db->get_one_record($qry);
		return $newchkResult;
	}
	public function updateAlbumVisitCounter($postData) {
		$qry = "SELECT ID, VISIT_COUNTER FROM dp_albums WHERE ID = " . $postData->ALBUM_ID. "";
		$chkResult = $this->db->get_one_record($qry);
		$updatePtr = $this->db->set_multiple_fields('dp_albums', 'VISIT_COUNTER = "' . ($chkResult['VISIT_COUNTER'] + 1) . '"', 'ID = "' . $chkResult['ID'] . '"');
		$chkSqlResult = $this->db->get_one_record($qry);
		return $chkSqlResult;
	}
	public function checkForPrivateAlbum($postData) {
		$ret = '';
		$adminKey = $this->decode($postData->ADMIN_KEY);
		$albumAccessKey = $postData->ALBUM_ACCESS_KEY;
		$albumID = $postData->ALBUM_ID;

		$sql = "SELECT * FROM dp_album_key WHERE ADMIN_KEY = '" . $adminKey . "'";
		$chkSqlPtr = $this->db->get_sql_exec($sql);
		$chkSqlRows = $this->db->get_db_num_rows($chkSqlPtr);
		$chkSqlResult = $this->db->get_one_record($sql);
		
		if($chkSqlRows == 1){
			$qry = "SELECT ID, EXPIRE_COUNTER FROM dp_albums WHERE ID = " . $albumID . " AND ALBUM_ACCESS_KEY = '" . $albumAccessKey . "'";
			$chkQryPtr = $this->db->get_sql_exec($qry);
			$chkQryRows = $this->db->get_db_num_rows($chkQryPtr);
			if($chkQryRows == 1) {
				$chkResult = $this->db->get_one_record($qry);
				if($chkResult['EXPIRE_COUNTER'] > $chkSqlResult['ACCESS_LIMIT']){
					$newAlbumAccessKey = $this->generateNewAlbumAccessKey();
					$newExpireCounter = 0;
					$updatePtr = $this->db->set_multiple_fields('dp_albums', 'EXPIRE_COUNTER = "' . $newExpireCounter . '",  ALBUM_ACCESS_KEY = "' . $newAlbumAccessKey . '"', 'ID = "' . $chkResult['ID'] . '"');				
				}else{			
					$updatePtr = $this->db->set_multiple_fields('dp_albums', 'EXPIRE_COUNTER = "' . ($chkResult['EXPIRE_COUNTER'] + 1) . '"', 'ID = "' . $chkResult['ID'] . '"');
				}
				$ret = true;
			} else {
				$ret = false;
	...
	public function addDbRecordsOfPhotosInAlbum($albumId,$fileName){
		...
		$chkSqlRows = $this->db->get_db_num_rows($chkSqlPtr);
		if($chkSqlRows == 0){
			$qry = "INSERT INTO dp_albums_photos (`ID`, `ALBUM_ID`, `IMAGE_THUMB_URL`,`UPLOADED_DATE`,`PHOTO_STATUS`, `FILE_TYPE` )  VALUES 
			(NULL, '" . $albumId . "', '" . trim($fileName) . "','" . $currDate . "','Y', 'photo')";
			$ptr = 	$this->db->get_sql_exec($qry);
		}
	}
	public function addDbRecordsOfVideosInAlbum($albumId,$fileName){
		$currDate = date('Y-m-d h:i:s');
		$sql = "SELECT ID  
				 FROM `dp_albums_photos` 
				 WHERE ALBUM_ID = " . $albumId . " AND LOWER(IMAGE_THUMB_URL) = '" . strtolower(trim($fileName)) . "'"; //die;
		...
		if($chkSqlRows == 0){
			$qry = "INSERT INTO dp_albums_photos (`ID`, `ALBUM_ID`, `IMAGE_THUMB_URL`,`UPLOADED_DATE`,`PHOTO_STATUS`, `FILE_TYPE` )  VALUES 
			(NULL, '" . $albumId . "', '" . trim($fileName) . "','" . $currDate . "','Y', 'video')";
			$ptr = 	$this->db->get_sql_exec($qry);
		}
	}
	public function getAlbumsPhotosList($postData){
		...
		if($chkSqlRows == 1){
			...
			$sql = "SELECT * FROM dp_albums_photos WHERE ALBUM_ID = " . $postData->ALBUM_ID . " AND FILE_TYPE = 'photo'";
			if($postData->DISPLAY == 'FE'){
	...
	public function getAlbumsVideoList($postData){
		$result = array();
		// select query
		// $sql = "SELECT * FROM dp_albums WHERE ID = " . $postData->ALBUM_ID . " AND ALBUM_FOLDER_TITLE = '" . $postData->ALBUM_FOLDER_TITLE . "'";
		$sql = "SELECT * FROM dp_albums WHERE ID = " . $postData->ALBUM_ID;
		...
		if($chkSqlRows == 1){
			$chkResult = $this->db->get_one_record($sql);
			$result[] = $chkResult['ALBUM_DISPLAY_TITLE'];
			$sql = "SELECT * FROM dp_albums_photos WHERE ALBUM_ID = " . $postData->ALBUM_ID . " AND FILE_TYPE = 'video'";
			if($postData->DISPLAY == 'FE'){
				$sql .= " AND PHOTO_STATUS = 'Y'";
			}
			$sql .= " ORDER BY ID DESC";
	...
	public function uploadAlbumVideos(){
		// ini_set('max_execution_time', '300');
		ini_set('max_execution_time', 0);
		$this->cors();
		$albumFolderPath = HTDOCS_PATH . '/images/albums/' . $_REQUEST['albumIntTitle'];
		if(chmod($albumFolderPath, 0777)) {
			$tmpMsg = '';
			$albumId = $_REQUEST['albumId'];
			$newFileName = date('Ymdhms') . $_FILES["myFiles"]['name'];
			if(move_uploaded_file($_FILES["myFiles"]["tmp_name"], $albumFolderPath . "/" . $newFileName)){
				$this->addDbRecordsOfVideosInAlbum($albumId,$newFileName);
				$tmpMsg = array($newFileName . ' Success');
			}else{
				$tmpMsg = array($newFileName . ' Failed');
			}
	...
	public function getReactComponentDisplaySettings($postData){
		$result = array();
		$sql = "SELECT * FROM dp_yt_display_settings WHERE DISPLAY_NAME = '" . $postData->DISPLAY_NAME . "'";
	...
	public function getRandomSongsForAlbum($postData) {
		$result = array();
		$returnArr = array();
		$requiredPlayingDuration = $postData->albumLength * 8;
		$limitValue = 1;
		if($requiredPlayingDuration > 180) {
			$limitValue = ceil($requiredPlayingDuration / 180);
		}
		$sql = "SELECT SONG_TITLE FROM dp_album_songs WHERE SONG_STATUS = 'Y' ORDER BY RAND() LIMIT " . $limitValue;
		$chkSqlPtr = $this->db->get_sql_exec($sql);
		$chkSqlRows = $this->db->get_db_num_rows($chkSqlPtr);
		...
		foreach($result as $rec){
			array_push($returnArr, $rec['SONG_TITLE']);
		}
		return $returnArr;		
	}
}