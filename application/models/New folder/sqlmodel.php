...
class sqlmodel extends CI_Model
{
	...
	*/
	...
	function updateStatisticOfCurrentVideo($videoId)
	{
		//CALLING API FOR VIDEO DETAILS
		$data = array();
		$videoDetails = file_get_contents('https://www.googleapis.com/youtube/v3/videos?part=statistics&id=' . $videoId .'&key=' . DP_YT_API_KEY);
		$videoDetails = json_decode($videoDetails);
		$videoDetails = $this->objectToArray($videoDetails);
		...
		$data = array();
		$allCommentsForVideoId = file_get_contents('https://www.googleapis.com/youtube/v3/commentThreads?key=' . DP_YT_API_KEY . '&textFormat=plainText&part=snippet&videoId=' . $videoId . '&maxResults=500');
	...
	function getVideoIds(){
		$funcId = $this->_setFunctionHistory(__method__);	
		$sql = "SELECT VIDEO_ID FROM `dp_yt_videos_master`";
		$result = $this->db->get_multiple_tables_records($sql);
		$this->_getExecutionTime($funcId); 
		return $result;
	}
	...
	*/	
	function getRecentYtVideos(){
		$funcId = $this->_setFunctionHistory(__method__);	
		$sql = "SELECT * FROM `dp_yt_videos_master` ORDER BY VIDEO_PUBLISH_DATE DESC LIMIT " . $this->getDisplaySetting('RECENT_VIDEOS_COUNT_HOMEPAGE'); //die;
		$result = $this->db->get_multiple_tables_records($sql);
		$this->_getExecutionTime($funcId); 
		return $result;
	}
	function getMidAgedVideos(){
		$funcId = $this->_setFunctionHistory(__method__);	
		$sql = "SELECT * FROM `dp_yt_videos_master` ORDER BY VIDEO_PUBLISH_DATE DESC LIMIT " . $this->getDisplaySetting('RECENT_VIDEOS_COUNT_HOMEPAGE') . "," .  $this->getDisplaySetting('MIDAGED_VIDEOS_COUNT'); //die;
		$result = $this->db->get_multiple_tables_records($sql);
		$this->_getExecutionTime($funcId); 
		return $result;
	}	
	...
	function getMostViewedRandomVideos(){
		...
		$sql = "
		SELECT * FROM (
			SELECT * FROM `dp_yt_videos_master` ORDER BY RAND() LIMIT " . $this->getDisplaySetting('MOSTVIEWED_VIDEOS_COUNT_HOMEPAGE') . " 
			) u
	...
	function getListOfVideosInPlaylist($playlistId,$extraVideoId='') {
		...
		$sql = "SELECT vm.*, pm.PLAYLIST_ID,pm.PLAYLIST_TITLE,pm.PLAYLIST_DP_URL, pv.VIDEO_POS FROM `dp_yt_videos_master` as vm,`dp_yt_playlists_master` as pm, `dp_yt_playlist_videos` as pv
		WHERE vm.VIDEO_ID = '" . $extraVideoId . "' 
		AND pm.PLAYLIST_ID = pv.PLAYLIST_ID 
	...
	function getAllVideosDetails($video_id) {
		$funcId = $this->_setFunctionHistory(__method__);	
		$chkStr = "SELECT * FROM `dp_yt_videos_master` 
		WHERE VIDEO_ID = '" . $video_id . "'";
	...
	function getRecentYtVideosForVideoPlay() {
		$funcId = $this->_setFunctionHistory(__method__);	
		$sql = "SELECT * FROM `dp_yt_videos_master` ORDER BY VIDEO_PUBLISH_DATE DESC LIMIT " . $this->getDisplaySetting('RECENT_VIDEOS_COUNT_VIDEOPLAY'); //die;
		$result = $this->db->get_multiple_tables_records($sql);
		$this->_getExecutionTime($funcId); 
		return $result;
	}
	function getMostViewedRandomVideosForVideoPlay() {
		...
		$sql = "
		SELECT * FROM (
			SELECT * FROM `dp_yt_videos_master` ORDER BY RAND() LIMIT " . $this->getDisplaySetting('MOSTVIEWED_VIDEOS_COUNT_VIDEOPLAY') . " 
			) u
		ORDER BY VIDEO_VIEWS DESC";
		$result = $this->db->get_multiple_tables_records($sql);
		$this->_getExecutionTime($funcId); 
