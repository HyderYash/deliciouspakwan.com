...
function _getCategoryHeading($title='Yash', $size='big'){
	$tmp = '';
	if($size == 'big'){
	...
	$tmp .= '<h2 style="' . $style . '"><span style="border-radius: 5px;padding:5px;font-size:16px;background: linear-gradient(120deg, #8fe7e39e 0%, #ebcde7 100%);">' . $title . '</span></h2>';
...
function _getYtVideoTitle($hrefUrl,$title ){
	$tmp = '';
	$tmp .= '<h3 id="title1"><a href="' . $hrefUrl .'" title="' . $title .'">' . $title . '</a></h3>';
	return $tmp;
}
function _getVideoSmallDesc($desc,$title,$hrefUrl){
	$tmp = '';
	$tmp .= '<p style="font-size:12px !important;color:darkblue;margin-top:5px;">';
		if($desc == ''){
...
function _getAuthorComments($desc){
	$tmp = '';
					...
					foreach($recentVideos as $rec){
						...
						$tmp .= '<div class="blog-content">';
							...
							$tmp .= '<div class="blog-title">';
								$tmp .= _getYtVideoTitle($randomSiteUrl,$rec['VIDEO_TITLE']);
							$tmp .= '</div>';
...
function _getListOfPlaylists($listOfPlaylist) {
	$CI =& get_instance();
	$tmp = '';
	foreach($listOfPlaylist as $rec) {
...
function displayBigZoominCards($iframe='no',$rec,$forWhat='video') {
	...
	$tmp .= '<div class="post-content">';
		...
		$tmp .= '<div class="post-title">';
			$tmp .= _getYtVideoTitle($videoDpUrl,$rec['VIDEO_TITLE']);
		...
		$tmp .= '<div style="width: 100%; min-height: 100px; overflow-y: scroll;">';
			...
			foreach($showCommentOfCurrentVideos as $rec){
				$tmp .= '<div class="post-content">';
					...
					$tmp .= _getAuthorComments($rec['COMMENT']);
				$tmp .= '</div>';		
			}
			$tmp .= '</div>';
		$tmp .= '</div>';
